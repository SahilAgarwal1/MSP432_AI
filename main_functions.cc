...
void loop() {
  ...
  uint8_t score = 0;
  volatile bool is_new_command = false;
  uint8_t scores[kCategoryCount];
  TfLiteStatus process_status = recognizer->ProcessLatestResults(
      output, current_time, &found_command, &score, &is_new_command, scores);
  ...
  uint32_t end = g_latest_audio_timestamp - start;
  start = g_latest_audio_timestamp;
  RespondToCommand(error_reporter, current_time, found_command, score,
                   is_new_command, scores);
}
